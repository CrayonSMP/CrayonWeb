...
import { assetData, type AssetItem, type AssetType, type PreviewType } from '../data/assets';
...
const Assets = () => {
    ...
    const TypeBadge: React.FC<{ type: AssetType }> = ({type}) => {
        ...
        return (
            <span className={`px-2 py-0.5 rounded text-sm font-semibold text-white uppercase ${colorMap[type]}`}>
          {type}
        </span>
        );
    };
    ...
    const AssetPreview: React.FC<{ preview: PreviewType }> = ({preview}) => {
        if (preview.type === 'image') {
            ...
            return (
                <div
                    className="w-full h-32 flex items-center justify-center bg-gray-800 rounded-lg"
                >
        ...
        return (
            <main className="flex-1 w-full max-w-6xl mx-auto px-4 md:px-8">
                ...
                <header className="py-16 md:py-24 text-center" data-aos="fade-up">
                    <FileIcon className="w-12 h-12 mx-auto mb-4 text-[var(--accent-color)]"/>
                    <h1 className="text-5xl md:text-6xl font-extrabold mb-4">
                        Official <span className="accent-gradient-text">Assets</span>
                    </h1>
                    <p className="text-xl text-gray-400 max-w-3xl mx-auto">
                        All official Crayon logos, brand kits, and style guides for media and partners for direct
                        download.
                    </p>
                </header>
                ...
                <section className="mb-16">
                    <div className="assets-grid">
                        {assetData.map((asset: AssetItem, index) => (
                            <div key={asset.id} className="pioneer-card-enhanced" data-aos="zoom-in"
